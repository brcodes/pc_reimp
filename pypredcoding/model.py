...
class PredictiveCodingClassifier:
    ...
    def predict_one_img(self,img_vec_2D):
        ...
        X_flat = data.flatten_images(img_vec_2D[None,:,:])
        # print("Xflat is")
        # print(X_flat.shape)
    ...
    def predict_mult_imgs(self,img_vec_3D):
        ...
        X_flat = data.flatten_images(img_vec_3D)
    ...
    def eval_online_reconstruction(self,image):
        ...
        plt.subplot(132),plt.imshow(fU1r1_resize_l1, cmap='Greys'),plt.title('image #{} L{}'.format(i+1,1))
        plt.xticks([]), plt.yticks([])
        # plt.colorbar(fraction=0.046, pad=0.04)
        plt.subplot(133),plt.imshow(fU1r1_resize_l2, cmap='Greys'),plt.title('image #{} L{}'.format(i+1,2))
        plt.xticks([]), plt.yticks([])
        # plt.colorbar(fraction=0.046, pad=0.04)
        plt.show()
    ...
    def eval_one_img(self,img_vec_2D,label,class_type='C2'):
        ...
        for i in range(0,self.n_eval_images):
            ...
            classif_hit_or_miss = eval_classification_check(label,predicted_img)
    ...
    def eval_mult_imgs(self,img_vec_3D,labels,class_type='C2'):
        ...
        for i in range(0,self.n_eval_images):
            print("eval image{}".format(i+1))
            image = img_vec_3D[i,:,:]
            predicted_img = self.predict(img_vec_dims=2,image)[0]
    ...
    def train(self,X,Y,class_type='C2'):
        ...
        for epoch in range(0,self.p.num_epochs):
            ...
            # print('n training images is {}'.format(self.n_training_images))
            ...
            for image in range(0, self.n_training_images):
                ...
                # # Calculate initial representation loss for every image, before image data
                # # is used in layer updates
                # E0 = self.rep_cost()
                # print('E0 image {} is {}'.format(image+1,E0))
            ...
            print('Eavg per epoch {} is {}'.format(epoch+1,E_avg_per_epoch))
    ...
    def predict(self,X,img_vec_dims=2,n_pred_updates=100):
        '''
        ...
        self.n_pred_updates = n_pred_updates
